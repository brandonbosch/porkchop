...
**Trust — Phase 3 (the point of the tool).** `e` expands the fold under the cursor **in place** (dimmed, "the model called this noise"), `e` again re-collapses; `E` expands all. `a` flips to an **audit view** showing *only* what was elided, grouped by file. This is where the original content surfaces — on demand, in flow. No web viewer, pager, or meat itself offers this.
...
Keybindings, JetBrains-flavored: `j/k` scroll, `J/K` or `n/p` hunk, `]/[` file, `tab` focus, `e`/`E` fold, `a` audit, `/` search, `u` unified toggle, `q` quit. Mouse wheel works for free in Bubble Tea.
...
**Phase 2 — JetBrains-class presentation.** Old │ new split alignment + intra-line token highlighting (the delegated pure module), `u` unified/split toggle, and navigation (hunk/file jumping, file strip, search). *Exit: review a 15-file change with the keyboard only; changed tokens pop.*

**Phase 3 — trust features (the product).** Plan sidecar via approach (a) (reference: `elision.go` `retainedDiffStats`); fold expansion (`e`/`E`, in place, dimmed); audit view (`a`). Original content surfaces here, on demand. *Exit: answer "what did it hide, and was it right?" in under a minute on any commit.*
